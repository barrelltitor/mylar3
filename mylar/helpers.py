        ...
        if queue.qsize() >= 1:
            ...
            if item == 'startup':
                try:
                    pending_jd2 = myDB.select(
                        "SELECT * FROM ddl_info WHERE jd2_job_id IS NOT NULL AND status IN (?, ?)",
                        ('JD2-Submitted', 'Queued'),
                    )
                    ...
                    for job in pending_jd2 or []:
                        job_data = dict(job)
                        logger.info('[JD2-QUEUE] Repopulating JD2 monitor queue with %s', job_data)
            ...
            if jd2_client is not None and job_id:
                try:
                    ...
                    completed_states = {'FINISHED', 'Finished', 'finished', 'DONE', 'Done', 'Extraction OK'}
                    ...
                    if job_status in completed_states:
                        ...
                        logger.info('[JD2-QUEUE] Job %s completed; status marked as Downloaded.', job_id)
                        ...
                        if mylar.CONFIG.POST_PROCESSING is True:
                            ...
                            temp_filename = item.get('filename') or '%s (%s) - %s' % (series, year, record_id)

                            nzb_folder = None
                            if dest_root and temp_filename:
                                folder_name = f"{temp_filename} - {record_id}" if record_id else temp_filename
                                nzb_folder = os.path.join(dest_root, folder_name)
                            elif dest_root:
                                nzb_folder = dest_root
                            ...
                            if nzb_folder:
                                try:
                                    ...
                                    logger.info('[JD2-QUEUE] Submitted job %s for post-processing (folder: %s).', job_id, nzb_folder)
                                except Exception as err:
                                    logger.warn('[JD2-QUEUE] Unable to enqueue job %s for post-processing: %s', job_id, err)
                            else:
                                logger.warn('[JD2-QUEUE] Job %s completed but JD2 destination is not configured; skipping post-processing enqueue.', job_id)
                        else:
                            logger.info('[JD2-QUEUE] Post-processing disabled; JD2 download retained for record %s.', record_id)
                        continue
                    ...
                    if job_status in failed_states:
                        ...
                        logger.warn('[JD2-QUEUE] Job %s reported failure state (%s).', job_id, job_status)
                        continue
